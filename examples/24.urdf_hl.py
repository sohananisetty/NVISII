...
nv.initialize(headless=True, lazy_updates=True)
...
# while 1:
for k in range(int(opt.nb_frames)):
  print(i)
  t = t + 0.01
...
#     p.addUserDebugLine(prevPose1, ls[4], [1, 0, 0], 1, trailDuration)
  ...
  prevPose1 = ls[4]
  hasPrevPose = 1
  print(f'rendering frame {str(k).zfill(5)}/{str(opt.nb_frames).zfill(5)}')
  nv.render_to_file(
        width=int(opt.width), 
        height=int(opt.height), 
        samples_per_pixel=int(opt.spp),
        file_path=f"{opt.outf}/{str(k).zfill(5)}.png"
    )
